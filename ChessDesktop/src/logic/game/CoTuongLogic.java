...
import logic.game.bean.Bookmark;
import logic.game.bean.History;
import logic.game.bean.LogicConstant;
import logic.game.bean.MapInfo;
import logic.game.bean.MyPiece;
import logic.game.bean.PiecesInfo;
import logic.game.piece.Piece;
import logic.game.piece.PieceManager;


public class CoTuongLogic {
    protected PieceManager[] playerPM = new PieceManager[2];
    ...
    public void initNewGame(int firstTurn, int timePlay) {
        timeStart = -1;
        MaxTime = timePlay;
    ...
    // kiem tra xem tuo'ng cua playerID co bi chieu ko? ( true = bi chie'u, false = ko)

    private boolean kiemTraChieuTuong(int PlayerID) {
        int i;
    ...
    // tinh toan nhung o ma cac quan co cua playerID co the di trong luo.t ke tiep
        ...
        for (i = 0; i < playerPM[PlayerID].iNumPiece; i++) {
            if (!playerPM[PlayerID].Pieces[i].isDead) {
                ...
                for (j = 0; j < maxTarget; j++) {
                    ...
                    if (kq.compareTo(Piece.ETargetType.e_DI_DUOC) < 0) {
                        list.add(j);
                    }
                }
                for (int a : list) {
    ...
    public Piece.ETargetType checkPlayerMovePiece(int PlayerID, int PieceID, int ToRow, int ToCol) {
    ...
    public void changePieceMapSide(int sidePlayer0) { // side cua thang playerID = 0
        int i;
        int OtherSide = 0;
        if (sidePlayer0 == 0) {
            OtherSide = 1;
